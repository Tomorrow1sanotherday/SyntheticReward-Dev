...
def save_images_and_rewards(images, aesthetic_scores, prompts, local_file_path="rewards_sd1.5_a_program.json"):
    """
    保存生成的图像和对应的美学评分
    """
    to_pil = ToPILImage()
    ...
    # 创建保存图像的文件夹
    if not os.path.exists('train_generated_images/sd1.5_a_program'):
        os.makedirs('train_generated_images/sd1.5_a_program')

    # 计算当前文件夹已有的图像数量，防止文件重名
    existing_images = len([f for f in os.listdir('train_generated_images/sd1.5_a_program') if f.endswith('.png')])
    ...
    for i, image in enumerate(images):
        ...
        # 确保图像命名唯一，避免覆盖现有图像
        save_image_path = f"train_generated_images/sd1.5_a_program/image_{existing_images + i + 1}.png"
...
def image_outputs_logger(image_data, global_step, accelerate_logger):
    ...
    # 调用保存图片和美学评分的函数
    save_images_and_rewards(images, rewards, prompts, local_file_path="rewards_sd1.5_a_program.json")
...
if __name__ == "__main__":
    ...
    training_args.project_kwargs = {
        ...
        "total_limit": 5,
        "project_dir": "./save_sd1.5_a_program",
    }

    training_args.log_with = "wandb"
    wandb.init(project="sd1.5_a_program", config=script_args)

    # 删除现有的检查点目录并重新创建
    checkpoint_dir = './save_sd1.5_a_program/checkpoints/'
