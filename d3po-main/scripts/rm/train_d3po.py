...
from vqa_feedback import ImageQAModel, VQAEnsembler, WeakSupervisor, Pipeline
from vqa_feedback import Logger as Local_logger
...
vqa_pipeline = Pipeline(vqa_ensembler, weak_supervisor, local_logger)
...
def main(_):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="d3po-pytorch-2", config=config.to_dict(), init_kwargs={"wandb": {"name": config.run_name}}
        )
    logger.info(f"\n{config}")
    ...
    reward_fn = getattr(d3po_pytorch.rewards, config.reward_fn)(vqa_pipeline)
