...
def combined_scorer(vqa_pipeline, aesthetic_scorer, local_file_path="ddpo_sd1.5_v+a_mms_rewards.json"):
    """
    结合VQA和美学评分的评分器
    """
...
if __name__ == "__main__":
    ...
    logger = Logger(log_file="ddpo_v+a_mms_traininglog.json", image_dir="train_generated_images/ddpo_sd1.5_v+a_mms")
    ...
    training_args.project_kwargs = {
        ...
        "total_limit": 5,
        "project_dir": "./ddpo_sd1.5_v+a_mms_save",
    }

    training_args.log_with = "wandb"
    wandb.init(project="ddpo_sd1.5_v+a_mms", config=script_args)
    ...
    trainer = DDPOTrainer(
        training_args,
        combined_scorer(vqa_pipeline, aesthetic_model, local_file_path="ddpo_sd1.5_v+a_mms_rewards.json"),
        prompt_fn,
        pipeline,
        image_samples_hook=image_outputs_logger,
    )

    checkpoint_dir = './ddpo_sd1.5_v+a_mms_save/checkpoints/'
