...
def combined_scorer(vqa_pipeline, aesthetic_scorer, local_file_path="rewards_sd1.5_v+a_program.json"):
    """
    结合VQA和美学评分的评分器
    """
    def _fn(images, prompts, metadata):
        ...
        for i, image in enumerate(images):
            ...
            vqa_score = vqa_score * 10  # 将0-1范围调整为0-10
            
            # 结合两个评分
            combined_score = (vqa_score + aesthetic_scores[i].item()) / 2
...
if __name__ == "__main__":
    ...
    logger = Logger(log_file="training_log_v+a_program.json", image_dir="train_generated_images/sd1.5_v+a_program")
    ...
    training_args.project_kwargs = {
        ...
        "total_limit": 5,
        "project_dir": "./save_sd1.5_v+a_program",
    }

    training_args.log_with = "wandb"
    wandb.init(project="sd1.5_v+a_program", config=script_args)
    ...
    trainer = DDPOTrainer(
        training_args,
        combined_scorer(vqa_pipeline, aesthetic_model, local_file_path="rewards_sd1.5_v+a.json"),
        prompt_fn,
        pipeline,
        image_samples_hook=image_outputs_logger,
    )

    checkpoint_dir = './save_sd1.5_v+a_program/checkpoints/'
