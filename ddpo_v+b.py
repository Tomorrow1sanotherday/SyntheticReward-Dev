...
# 临时登录并设置 API 密钥
# wandb.login(key="81923d61e4ecee582313395ee7185cebab2a38fe")  # 将 API 密钥传递给这个方法
...
def combined_scorer(vqa_pipeline, local_file_path="ddpo_sd1.5_v+b_mms_rewards.json"):
    ...
    def _fn(images, prompts, metadata):
        ...
        for i, image in enumerate(images):
            ...
            rewards = [vqa_score, bert_score]
            
            # 结合两个评分
            combined_score = reward_ensembler.normalize(rewards = rewards, method = 'weight', weights = [10, 10])
...
if __name__ == "__main__":
    ...
    logger = Logger(log_file="ddpo_sd1.5_v+b_mms_traininglog.json", image_dir="train_generated_images/ddpo_sd1.5_v+b_mms")
    ...
    training_args.project_kwargs = {
        ...
        "total_limit": 5,
        "project_dir": "./ddpo_sd1.5_v+b_mms_save",
    }

    training_args.log_with = "wandb"
    wandb.init(project="ddpo_sd1.5_v+b_mms", config=script_args)
    ...
    trainer = DDPOTrainer(
        training_args,
        combined_scorer(vqa_pipeline=vqa_pipeline, local_file_path="ddpo_sd1.5_v+b_mms_rewards.json"),  # 使用合并的评分函数
        prompt_fn,
        pipeline,
        image_samples_hook=image_outputs_logger,
    )

    checkpoint_dir = './ddpo_sd1.5_v+b_mms_save/checkpoints/'
