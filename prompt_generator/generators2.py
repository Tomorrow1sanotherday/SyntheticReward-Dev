...
class DeepSeekAttributeManager:
    ...
    async def determine_object_attributes(self,
        ...
        """Query DeepSeek API to determine appropriate attributes for a specific object. Retries indefinitely until successful."""
...
Examples:[breed, color, size, ...]"""
        ...
        wait_time = 0 # Wait 5 seconds between retries
        ...
        while retries < max_retries:
            try:
                async with session.post(
                    ...
                    headers=self.headers,
                    json=payload,
                    timeout=aiohttp.ClientTimeout(total=60) # Increased timeout slightly
                ) as response:
                    response.raise_for_status() # Raise HTTPError for bad responses (4xx or 5xx)
                    result = await response.json()

                    if result is None or "choices" not in result or not result["choices"]:
                        print(f"Warning: API returned invalid attribute list structure for {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                        retries += 1
                        await asyncio.sleep(wait_time)
                        continue # Retry

                    # Parse JSON returned by API
                    attributes_text = result["choices"][0]["message"]["content"]
                    # print(f"API response (attributes for {object_name}): {attributes_text}") # Verbose logging
                    try:
                        attributes_data = json.loads(attributes_text)
                        attributes_list = []

                        # Try to extract the list from various possible structures
                            ...
                            if not found:
                                for value in attributes_data.values():
                                     if isinstance(value, list):
                                         ...
                                         break
                            if not found:
                                print(f"Warning: Cannot extract attribute list from API response dict for {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                                retries += 1
                                await asyncio.sleep(wait_time)
                                continue # Retry
                        else:
                             print(f"Warning: Unexpected API response format for attributes of {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                             retries += 1
                             await asyncio.sleep(wait_time)
                             continue # Retry
                        ...
                        if not attributes_list or len(attributes_list) < 3:
                            print(f"Warning: Attribute list returned by API for {object_name} is empty or too short ({len(attributes_list)}) (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                            retries += 1
                            await asyncio.sleep(wait_time)
                            continue # Retry
                        ...
                        print(f"Warning: Cannot parse JSON returned for {object_name}: '{attributes_text}' (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                        retries += 1
                        await asyncio.sleep(wait_time)
                        continue # Retry
    ...
    async def generate_attribute_values(self,
                                     ...
                                     attribute_name: str,
                                     count: int) -> List[str]:
        """Use DeepSeek API to generate multiple possible values for a specific attribute. Retries indefinitely until successful."""
...
Examples:["oak", "pine", ...]"""
        ...
        wait_time = 0 # Wait 3 seconds between retries for value generation
        ...
        while retries < max_retries:
            try:
                ...
                if result is None or "choices" not in result or not result["choices"]:
                    print(f"Warning: API returned invalid result structure for {attribute_name} of {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                    retries += 1
                    await asyncio.sleep(wait_time)
                    continue # Retry

                values_text = result["choices"][0]["message"]["content"]
                # print(f"API response (values for {object_name} -> {attribute_name}): {values_text}") # Verbose logging
                try:
                        ...
                        if not found:
                             for value in values_data.values():
                                 if isinstance(value, list):
                                     ...
                                     break
                        if not found:
                            print(f"Warning: Unable to extract value list from API response dict for {attribute_name} of {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                            retries += 1
                            await asyncio.sleep(wait_time)
                            continue # Retry
                    else:
                        print(f"Warning: Unexpected API response format for values of {attribute_name} of {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                        retries += 1
                        await asyncio.sleep(wait_time)
                        continue # Retry
                    ...
                    if not values_list:
                        print(f"Warning: API returned empty list for {attribute_name} of {object_name} (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                    ...
                    print(f"Warning: Cannot parse JSON returned for {attribute_name} of {object_name}: '{values_text}' (Retry {retries+1}/{max_retries}), retrying in {wait_time} seconds...")
                    retries += 1
                    await asyncio.sleep(wait_time)
                    continue # Retry
...
class PromptGenerator:
    ...
    async def generate_natural_prompt(self, session: aiohttp.ClientSession,
                                   ...
                                   complexity: int) -> Optional[str]: # Return Optional[str]
        """
        Generate a natural sentence using DeepSeek API, respecting the semaphore.

        Returns:
            Generated natural sentence prompt or None if generation fails after retries.
        """
        if complexity == 0:
            # Handle simple case without API call based on attributes
            if attributes_used:
                 # If complexity is 0 but somehow attributes were selected (unlikely with current logic), form a simple prompt
                 attributes_str_simple = ", ".join(attributes_used.values())
                 return f"A {object_name} that is {attributes_str_simple}."
            else:
                 # Standard case for complexity 0: just the object name
...
"""
        payload = {
            ...
            "max_tokens": 150, # Slightly increased max_tokens
             # Consider adding frequency/presence penalties if repetition is an issue
            # "frequency_penalty": 0.1,
            # "presence_penalty": 0.1
        }

        max_retries = 10 # Reduced max retries slightly
        retry_delay = 0  # Slightly increased base delay

        async with semaphore: # Acquire semaphore before making the API call
             # print(f"Debug: Acquired semaphore for {object_name}, complexity {complexity}") # Optional debug log
             for attempt in range(max_retries):
                 try:
                     ...
                     async with session.post(
                         ...
                         # Log status for debugging rate limit or server issues
                         # print(f"Debug: API call for {object_name} status: {response.status}")
                         response.raise_for_status() # Check for HTTP errors (4xx, 5xx)
                         result = await response.json()

                         if not result or "choices" not in result or not result["choices"]:
                              print(f"Warning: Invalid API response structure for '{object_name}' (Attempt {attempt + 1}/{max_retries}). Retrying...")
                              # Short delay before retrying within the semaphore lock for API issues
                              await asyncio.sleep(retry_delay / 2)
                              continue # Go to next attempt

                         generated_text = result["choices"][0]["message"]["content"].strip()

                         # Basic cleanup: remove potential surrounding quotes or markdown backticks
                         if generated_text.startswith('"') and generated_text.endswith('"'):
                             generated_text = generated_text[1:-1]
                         if generated_text.startswith('`') and generated_text.endswith('`'):
                            generated_text = generated_text[1:-1]
                         # Remove potential "Output:" prefix
                         if generated_text.lower().startswith("output:"):
                              generated_text = generated_text[len("output:"):].strip()


                         # Basic validation - ensure it's not empty and doesn't look like an error message
                         if not generated_text or len(generated_text) < 3 or "sorry" in generated_text.lower() or "cannot fulfill" in generated_text.lower() or "error" in generated_text.lower():
                             print(f"Warning: Received potentially invalid/empty prompt for '{object_name}' (Attempt {attempt+1}/{max_retries}): '{generated_text}'. Retrying...")
                             # Optional: Add a short sleep even within semaphore if API gives bad results frequently
                             await asyncio.sleep(1)
                             continue
                         
                           # 添加额外的清理步骤，移除转义的双引号和其他转义字符
                         generated_text = generated_text.replace('\\"', '"')  # 替换转义的双引号
                         generated_text = generated_text.replace('\\n', ' ')  # 替换换行符为空格
                         generated_text = generated_text.replace('\\t', ' ')  # 替换制表符为空格
                         generated_text = generated_text.replace('\\\\', '\\')  # 处理双重转义的反斜杠
                         
                         # print(f"Debug: Released semaphore for {object_name}") # Optional debug log
                         return generated_text.strip() # Success
                     ...
                     if attempt < max_retries - 1:
                         ...
                         # Adjust delay based on error type if needed
                         if status_code == 429: # Rate limited
                              print(f"Info: Rate limit hit for '{object_name}'. Consider lowering concurrent_limit.")
                              current_delay = max(current_delay, 10) # Longer delay for rate limits
                         ...
                         await asyncio.sleep(current_delay) # Sleep *before* next attempt (outside semaphore lock implicitly)
                     else:
                         print(f"Error: Failed to generate prompt for '{object_name}' after {max_retries} attempts due to {error_msg}. Skipping.")
                         # print(f"Debug: Released semaphore for {object_name} after failure") # Optional debug log
                         return None # Indicate failure

                 except Exception as e:
                     # Catch other unexpected errors during API call or processing
                     ...
                     # print(f"Debug: Released semaphore for {object_name} after unexpected error") # Optional debug log
                     return None # Indicate failure

        # Fallthrough case: Should not be reached if logic is correct, but indicates failure
        print(f"Error: Prompt generation loop completed unexpectedly for '{object_name}'. Skipping.")
        # print(f"Debug: Released semaphore for {object_name} at loop end") # Optional debug log
        return None
        ...
        async with self.file_lock: # Use lock to ensure unique, sequential IDs and safe file write
            ...
            if output_file_handle:
                try:
                    ...
                    # Flushing frequently can impact performance, consider flushing less often or at the end
                    # output_file_handle.flush()
                except Exception as e:
                     print(f"Error: Failed to write prompt {current_total_prompt_id} for '{object_name}' to file: {e}")
                     # Decide if this should cause the task to "fail"
    ...
    async def generate_all_prompts(self,
        ...
        for category, objects in self.objects_data.items():
            ...
            for obj in objects:
                ...
                if available_attr_count == 0 and min_complexity > 0:
                    # print(f"Info: Object '{object_name}' (Cat: {category}) has no attributes. Skipping {prompts_per_object} potential prompts.")
                ...
                for i in range(prompts_per_object):
                    ...
                    # (This might happen if attribute lists contain only empty strings, etc.)
                    if actual_complexity > 0 and not attributes_used and available_attr_count > 0:
                         # print(f"Warning: Could not select attributes for '{object_name}' (Compl {actual_complexity}, Attempt {i+1}). Skipping this prompt instance.")
        ...
        # Let semaphore control concurrency, set connector limit higher or None
        conn = aiohttp.TCPConnector(limit=self.concurrent_limit * 2, ttl_dns_cache=300)
        # General timeout for session level operations (e.g., DNS resolution)
        ...
        try:
             async with aiohttp.ClientSession(connector=conn, timeout=timeout) as session:
                ...
                for job_info in prompt_jobs:
                    task = asyncio.create_task(
                        ...
                        # Optional: Name tasks for easier debugging if needed
                        # name=f"PromptGen_{job_info['category']}_{job_info['object_id']}_{job_info['prompt_index']}"
                    )
                    tasks.append(task)

                print(f"Launching {len(tasks)} generation tasks...")
                # Use tqdm_asyncio for progress bar with gather
                # REMOVED return_exceptions=True as it's not a valid arg for tqdm_asyncio.gather
                results = await tqdm_asyncio.gather(
                    *tasks,
                    desc="Generating Prompts",
                    total=len(tasks),
                    # Leave unit as 'prompt' or 'task'
                    unit="task"
                )
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        # Attempt to get task name if available (might not be set or standard)
                        ...
                        # Optionally log the full traceback for exceptions caught by gather
                        # traceback.print_exception(type(result), result, result.__traceback__)
                        total_prompts_failed += 1
                    elif result is None:
                        # Failure case handled within the task (already logged there)
                        total_prompts_failed += 1
            ...
            # Ensure file is closed even if setup fails
        ...
        # Final count should ideally match tasks launched = generated + failed
        ...
        # Note: self.total_prompt_id_counter holds the *next* ID, so subtract 1 for count
        print(f"Final global prompt ID assigned: {self.total_prompt_id_counter - 1}")
        print(f"Total time: {elapsed_time:.2f} seconds ({elapsed_time/60:.2f} minutes)")

        # Calculate and print estimated prompts per second if time > 0
        if elapsed_time > 0:
        ...
        if not output_file_handle:
            # Sort results by total_prompt_id if returning in memory
            all_prompts_data.sort(key=lambda x: x.get('total_prompt_id', 0))
            return all_prompts_data
        else:
            # Return empty list as results were streamed
            # Verify file existence and content as a final check
